...
│  Tools: run_task, get_task_info, get_task_logs,             │
│         wait_next_event, cleanup_task                        │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────┐
│                    microVM (vfkit + NixOS)                   │
│  - Isolated git repo via virtiofs                           │
...
| Module | Purpose |
|--------|---------|
| `server.py` | MCP entry point, tool registration via FastMCP |
| `tools.py` | `Orchestrator` class - coordinates task lifecycle |
| `core/task.py` | Task state machine (PENDING → RUNNING → COMPLETED/FAILED) |
| `core/vm.py` | VM process management, nix-build, environment setup |
| `core/events.py` | Thread-safe async event queue |
| `core/git.py` | Isolated repo setup, merge operations |

### Data Layout

```
.microvm/                           # Per-project config
├── config.json
└── tasks/{task_id}/               # Task workspace
    ├── task.json                  # Metadata
    ├── repo/                      # Isolated git repo
    ├── task.md                    # Task description
    ├── result.json                # Execution result
    └── merge-result.json          # Git merge outcome

~/.microvm-orchestrator/           # Global persistent storage
└── slots/{slot}/
    ├── var/                       # Persistent /var
    └── container-storage/         # Podman storage
```
